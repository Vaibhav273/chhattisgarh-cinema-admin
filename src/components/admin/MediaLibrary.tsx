...
interface MediaLibraryProps {
  ...
  title?: string;
  // 🔥 NEW: Optional context for logging
  sourceModule?: string; // e.g., "Movies", "Events", etc.
  sourceId?: string; // e.g., movieId, eventId
}
...
const MediaLibrary: React.FC<MediaLibraryProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      // 🔥 LOG: Media library opened
      if (sourceModule) {
        logActivity("media_library_opened", sourceModule, "", {
          mediaType,
  ...
  const handleSelect = () => {
    if (selectedMedia) {
      // 🔥 LOG: Media selected
      const selectedItem = filteredMedia.find(
        (item) => getDisplayUrl(item) === selectedMedia,
      );

      if (sourceModule && selectedItem) {
        logActivity("media_selected", sourceModule, "", {
          mediaName: selectedItem.name,
          mediaType: selectedItem.type,
          mediaUrl: selectedMedia,
          sourceId: sourceId || "unknown",
          cdnEnabled: cdnSettings?.enabled || false,
        });
      }
  ...
  useEffect(() => {
    if (searchQuery && sourceModule) {
      const timer = setTimeout(() => {
        logActivity("media_search", sourceModule, "", {
          searchQuery,
          filterType,

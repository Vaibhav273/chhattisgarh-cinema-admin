...
const AdminDashboard: React.FC = () => {
  ...
  const [logView, setLogView] = useState<"activity" | "system">("activity");
  ...
  const setupRealtimeAnalytics = () => {
    ...
    console.log(
      `📊 Setting up real-time analytics for last ${timeRange} days...`,
    );
  ...
  const getActivityLogIcon = (level: string) => {
    switch (level) {
      case "success":
        return "✅";
      case "error":
  ...
  const getActivityLogColor = (level: string) => {
    switch (level) {
      case "success":
  ...
  const fetchCounts = async () => {
    try {
      ...
      const totalRevenue = transactionsDocs.docs.reduce(
  ...
  const fetchPreviousMonthComparison = async (currentData: any) => {
    try {
      ...
      const prevStatsDoc = await getDoc(
        doc(db, "analytics", "monthly", "stats", prevMonthId),
      );
  ...
  const fetchRecentActivities = async () => {
    try {
      ...
      const activities: RecentActivity[] = activitiesSnapshot.docs.map(
        (doc) => {
          const data = doc.data();
          return {
            id: doc.id,
  ...
  const fetchWeeklyRevenue = async () => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dailyStatsDoc = await getDoc(
          doc(db, "analytics", "daily", "stats", dateStr),
        );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${stat.bgGradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300`}
              />
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${stat.bgGradient} flex items-center justify-center shadow-lg`}
                  >
                    <Icon size={24} className="text-white" />
                  </div>
                  <div
                    className={`flex items-center gap-1 px-2 py-1 rounded-full ${stat.change >= 0 ? "bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400"}`}
                  >
                    {stat.change >= 0 ? (
                      <ArrowUp size={14} />
                    ) : (
        ...
        {can(Permission.VIEW_REVENUE) && (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="px-4 py-2 bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400 rounded-xl text-sm font-semibold">
                Week
              </button>
            </div>
            <div className="flex items-end justify-between h-64 gap-4">
              {weeklyRevenue.map((day, index) => {
                const maxRevenue = Math.max(
                  ...weeklyRevenue.map((d) => d.revenue),
                );
                const height =
                  maxRevenue > 0 ? (day.revenue / maxRevenue) * 100 : 10;
              ...
              {analyticsData.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 shadow-lg border border-slate-200 dark:border-slate-800 text-center">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 mb-4">
                    Analytics data will appear here once your platform has
                    activity.
                  </p>
                  <p className="text-sm text-slate-500">
                    Firestore Path:{" "}
            ...
            <div className="space-y-4">
              {topContent.map((content, index) => (
                <motion.div
                  ...
                  className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
                >
                  <img
            ...
            <div className="space-y-3">
              {recentActivities.slice(0, 5).map((activity, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg ${activity.color} bg-opacity-10 flex items-center justify-center flex-shrink-0`}
                    >
        ...
        >
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {activityLogs.map((log, index) => (
                    <motion.div
                      ...
                      className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all cursor-pointer group"
                    >
                      {/* Icon */}
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${getActivityLogColor(log.level)}`}
                      >
                        <span className="text-lg">
                          {getActivityLogIcon(log.level)}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-bold text-purple-500 bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded">
                            {log.module}
                          </span>
                          {log.subModule && (
                        ...
                        {/* Message */}
                        <p className="text-sm text-slate-800 dark:text-white font-medium line-clamp-2 group-hover:line-clamp-none transition-all">
                          {log.message}
                        </p>
            ...
            <>
              {loadingLogs ? (
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {systemLogs.map((log, index) => (
                    ...
                    >
                      {/* Icon */}
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${getLogColor(log.action)}`}
                      >
                        <span className="text-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        {/* Module & SubModule */}
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-bold text-orange-500 bg-orange-100 dark:bg-orange-900/30 px-2 py-0.5 rounded">
                            {log.module}
                          </span>
                          {log.subModule && (
                            <span className="text-xs text-slate-500">
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-xs text-slate-400 mt-2">
                    System logs will appear here once system actions occur
                  </p>
                </div>
              )}
            </>
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-xl font-bold hover:shadow-lg transition-all"
            >
              View All {logView === "activity" ? "Activity" : "System"} Logs →
            </button>
          </div>
        </motion.div>
      </div>

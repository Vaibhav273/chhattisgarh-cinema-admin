...
const UsersManagement: React.FC = () => {
    ...
    try {
      if (alertModal.action === "delete") {
        ...
        await logUserManagementAction(
          "user_deleted",
          "success",
          `Deleted user: ${alertModal.userName}`,
          auth.currentUser,
          {
            targetUserId: alertModal.userId,
            targetUserName: alertModal.userName,
        ...
        await logUserManagementAction(
          "user_banned",
          "success",
          `Banned user: ${alertModal.userName}`,
          auth.currentUser,
          {
            targetUserId: alertModal.userId,
            targetUserName: alertModal.userName,
            bannedBy: userRole,
            actionType: "single_ban",
          },
        );
        ...
        await logUserManagementAction(
          "user_unbanned",
          "success",
          `Unbanned user: ${alertModal.userName}`,
          auth.currentUser,
          {
            targetUserId: alertModal.userId,
            targetUserName: alertModal.userName,
            unbannedBy: userRole,
            actionType: "single_unban",
          },
        );
        ...
        await logUserManagementAction(
          "users_bulk_deleted",
          "success",
          `Bulk deleted ${selectedUsers.size} users`,
          auth.currentUser,
          {
        ...
        await logUserManagementAction(
          "users_bulk_banned",
          "success",
          `Bulk banned ${selectedUsers.size} users`,
          auth.currentUser,
          {
            totalUsers: selectedUsers.size,
            userIds: userIds,
        ...
        setUsers((prev) =>
          prev.map((u) =>
            selectedUsers.has(u.uid) ? { ...u, status: "banned" } : u,
          ),
        );
        showToast(
        ...
        await logUserManagementAction(
          ...
          "success",
          `Bulk activated ${selectedUsers.size} users`,
          auth.currentUser,
          {
            totalUsers: selectedUsers.size,
            userIds: userIds,
        ...
        setUsers((prev) =>
          prev.map((u) =>
            selectedUsers.has(u.uid) ? { ...u, status: "active" } : u,
          ),
        );
        showToast(
          `${selectedUsers.size} users activated successfully!`,
          "success",
        );
        clearSelection();
      }
      ...
      await logUserManagementAction(
        `user_action_failed`,
        "failed",
        `Failed to ${alertModal.action}: ${error instanceof Error ? error.message : "Unknown error"}`,
        auth.currentUser,
        {
  ...
  // ✏️ EDIT USER
  // ═══════════════════════════════════════════════════════════════
  const handleEdit = async (userId: string, targetUserRole: UserRole) => {
    if (!canAssignRoles()) {
      showToast("You do not have permission to edit users", "error");
      return;
    }

    // ✅ Log navigation to edit page
    await logUserManagementAction(
      "user_edit_opened",
      "success",
      `Opened edit page for user: ${userId}`,
      auth.currentUser,
      {
        targetUserId: userId,
  ...
  const handleView = async (user: User) => {
    ...
    await logUserManagementAction(
      "user_details_viewed",
      "success",
      `Viewed details for user: ${user.displayName}`,
      auth.currentUser,
      {
  ...
  const exportToCSV = async () => {
    try {
      ...
      await logUserManagementAction(
        "users_export_failed",
        "failed",
        `Failed to export users: ${error instanceof Error ? error.message : "Unknown error"}`,
        auth.currentUser,
        {
          error: error instanceof Error ? error.message : "Unknown error",
        },
                    ...
                    {filteredUsers.map((user, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div
                              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r ${roleBadge.gradient} text-white font-bold text-xs shadow-lg`}
                            >
                              <RoleIcon size={14} />
                              {roleBadge.text}
                            </div>
                          ...
                          <td className="px-6 py-4">
                            <div
                              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg ${statusBadge.class} font-bold text-xs`}
                            >
                              <StatusIcon size={14} />
                              {statusBadge.text}
                            </div>

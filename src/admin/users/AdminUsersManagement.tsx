...
const AdminUsersManagement: React.FC = () => {
        ...
        setAllAdmins((prev) =>
          prev.map((a) =>
            a.uid === alertModal.adminId
              ? { ...a, status: "suspended", isActive: false }
              : a,
          ),
        );
  ...
  const departments = Array.from(
    new Set(allAdmins.map((a) => a.department).filter(Boolean)),
  ) as string[];
                  ...
                  {admins.map((admin, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-3 py-1 bg-gradient-to-r ${roleBadge.gradient} text-white rounded-full text-xs font-bold flex items-center gap-1 w-fit`}
                          >
                            <RoleIcon size={12} />
                            {roleBadge.text}
                          </span>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-3 py-1 ${statusBadge.class} rounded-full text-xs font-bold flex items-center gap-1 w-fit`}
                          >
                            <StatusIcon size={12} />
                            {statusBadge.text}
                          </span>
          ...
          >
            ...
            <p className="text-slate-500 dark:text-slate-500 mb-6">
              {searchTerm
                ...
                : "No admins match the selected filters"}
            </p>
          </motion.div>
        )}
      </div>

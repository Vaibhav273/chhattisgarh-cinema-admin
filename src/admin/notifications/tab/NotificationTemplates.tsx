...
import type { NotificationPriority, NotificationTemplate, NotificationType } from '../../../types/notification';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage'; // ✅ ADDED
import { storage } from '../../../config/firebase'; // ✅ ADDED
...
const TemplatesTab: React.FC = () => {
    ...
    const handleToggleActive = async (template: NotificationTemplate) => {
        try {
            ...
            showToast(template.isActive ? 'Template deactivated' : 'Template activated', 'success');
            fetchTemplates();
        } catch (error) {
        ...
        <div className="py-6 space-y-6">
            ...
            <AnimatePresence>
                {toast.show && (
                    ...
                    >
                        <div className={`px-6 py-4 rounded-2xl shadow-2xl backdrop-blur-xl flex items-center gap-3 min-w-[300px] ${toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white`}>
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <span className={`px-2 py-1 rounded-lg text-xs font-bold ${getTypeColor(template.type)}`}>
                                                {template.type.replace('_', ' ').toUpperCase()}
                                            </span>
                                            <span className={`px-2 py-1 rounded-lg text-xs font-bold ${getPriorityColor(template.priority)}`}>
                                    ...
                                    >
                                        <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all ${template.isActive ? 'left-6' : 'left-0.5'}`} />
                                    </button>
                                </div>
                            </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2">
                                        Title * <span className="text-xs text-gray-500">(Max 50 characters)</span>
                                    </label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2">
                                        Message * <span className="text-xs text-gray-500">(Max 200 characters)</span>
                                    </label>
                                    <textarea
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all ${formData.isActive ? 'left-6' : 'left-0.5'}`} />
                                    </button>

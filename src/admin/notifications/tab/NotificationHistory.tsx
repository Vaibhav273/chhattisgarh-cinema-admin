...
const NotificationHistoryTab: React.FC = () => {
    ...
    const getTypeColor = (type: string) => {
        const colors: Record<string, string> = {
            new_content: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400',
            ...
            promotion: 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400',
            system: 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400',
            content_update: 'bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400',
            ...
            announcement: 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400',
        };
        ...
        <div className="py-6 space-y-6">
            ...
            <AnimatePresence>
                {toast.show && (
                    ...
                    >
                        <div
                            className={`px-6 py-4 rounded-2xl shadow-2xl backdrop-blur-xl flex items-center gap-3 min-w-[300px] ${toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'
                                } text-white`}
                        >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div className="mt-4 text-sm text-slate-600 dark:text-slate-400">
                    Showing <span className="font-bold text-slate-800 dark:text-white">{filteredNotifications.length}</span> of{' '}
                    <span className="font-bold text-slate-800 dark:text-white">{notifications.length}</span> notifications
                </div>
            </div>
            ...
            {loading && notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64">
                    ...
                    />
                    <p className="text-slate-600 dark:text-slate-400 font-semibold mt-4">Loading notifications...</p>
                </div>
                ...
                >
                    ...
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No Notifications Found</h3>
                    <p className="text-slate-600 dark:text-slate-400">
                        {notifications.length === 0 ? 'Send your first notification to see it here' : 'Try adjusting your filters'}
                    </p>
                </motion.div>
            ) : (
                            ...
                            <div className="p-6">
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            <h4 className="font-bold text-slate-800 dark:text-white text-lg">{notification.title}</h4>
                                            <span className={`px-3 py-1 rounded-lg text-xs font-bold ${getTypeColor(notification.type)}`}>
                                                {notification.type.replace('_', ' ').toUpperCase()}
                                            </span>
                                            <span className={`px-3 py-1 rounded-lg text-xs font-bold ${getStatusColor(notification.status)}`}>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">Sent At</p>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-semibold text-slate-800 dark:text-white">{getTargetLabel(notification)}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">Read Count</p>
                                            <p className="text-sm font-semibold text-slate-800 dark:text-white">
                                    ...
                                    >
                                        <div className="p-6 space-y-4">
                                            {notification.imageUrl && (
                                                <div>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Image</p>
                                                    <img
                                            ...
                                            {notification.actionUrl && (
                                                <div>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Action URL</p>
                                            ...
                                            {notification.actionLabel && (
                                                <div>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Action Label</p>
                                                    <p className="text-sm text-slate-600 dark:text-slate-400">{notification.actionLabel}</p>
                                                </div>
                                            )}

                                            <div>
                                                <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Priority</p>
                                                <span
                                                    className={`px-3 py-1 rounded-lg text-xs font-bold ${notification.priority === 'urgent'
                                            ...
                                            {notification.createdByName && (
                                                <div>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Created By</p>
                                                    <p className="text-sm text-slate-600 dark:text-slate-400">{notification.createdByName}</p>
                                                </div>
                                            )}
                                        </div>
                                    </motion.div>

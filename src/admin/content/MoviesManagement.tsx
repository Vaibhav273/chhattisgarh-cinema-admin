...
const MoviesManagement: React.FC = () => {
    ...
    // ✏️ TOGGLE PUBLISH
    // ═══════════════════════════════════════════════════════════════
    const handleTogglePublish = async (movieId: string, currentStatus: boolean) => {
        if (!can(Permission.EDIT_ANY_CONTENT)) {
            showToast('You do not have permission to edit movies', 'error');
            return;
        }
        ...
        try {
            ...
            setMovies((prev) =>
                prev.map((m) =>
                    m.id === movieId ? { ...m, isPublished: !currentStatus } : m
                )
            );
            fetchStats();
            showToast(`Movie ${!currentStatus ? 'published' : 'unpublished'} successfully!`, 'success');
        } catch (error) {
    ...
    // ⭐ TOGGLE FEATURED
    // ═══════════════════════════════════════════════════════════════
    const handleToggleFeatured = async (movieId: string, currentStatus: boolean) => {
        if (!can(Permission.EDIT_ANY_CONTENT)) {
            showToast('You do not have permission to edit movies', 'error');
            return;
        }
        ...
        try {
            ...
            setMovies((prev) =>
                prev.map((m) =>
                    m.id === movieId ? { ...m, isFeatured: !currentStatus } : m
                )
            );
            fetchStats();
            showToast(`Movie ${!currentStatus ? 'featured' : 'unfeatured'} successfully!`, 'success');
        } catch (error) {
    ...
    const filteredMovies = movies.filter((movie) =>
        ...
        (movie.titleHindi && movie.titleHindi.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <AlertModal
                ...
                onClose={() => setAlertModal({ isOpen: false, movieId: '', movieTitle: '' })}
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-white/90 text-lg">Manage your entire movie collection</p>
                        </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4">
                    ...
                    />
                    <p className="text-slate-600 dark:text-slate-400 font-semibold">Loading movies...</p>
                </div>
                                    ...
                                    <img
                                        ...
                                        onError={(e) => {
                                            (e.target as HTMLImageElement).src =
                                                'https://via.placeholder.com/300x450/8b5cf6/ffffff?text=' + encodeURIComponent(movie.title);
                                        }}
                                    />
                                        ...
                                        {can(Permission.EDIT_ANY_CONTENT) && (
                                            <>
                                                <motion.button
                                                    ...
                                                    className={`flex-1 px-2 py-1 rounded-lg text-xs font-bold transition-all ${movie.isPublished
                                                ...
                                                <motion.button
                                                    whileHover={{ scale: 1.05 }}
                                                    whileTap={{ scale: 0.95 }}
                                                    onClick={() => handleToggleFeatured(movie.id, movie.isFeatured || false)}
                                                    className={`flex-1 px-2 py-1 rounded-lg text-xs font-bold transition-all ${movie.isFeatured
                ...
                >
                    ...
                    <h3 className="text-xl font-bold text-slate-600 dark:text-slate-400 mb-2">No Movies Found</h3>
                    <p className="text-slate-500 dark:text-slate-500 mb-6">
                        {searchTerm ? 'Try different search terms' : 'Start by adding your first movie'}
                    </p>

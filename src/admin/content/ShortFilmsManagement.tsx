...
const ShortFilmsManagement: React.FC = () => {
    ...
    // ✏️ TOGGLE PUBLISH
    // ═══════════════════════════════════════════════════════════════
    const handleTogglePublish = async (filmId: string, currentStatus: boolean) => {
        if (!can(Permission.EDIT_ANY_CONTENT)) {
            showToast('You do not have permission to edit short films', 'error');
            return;
        }
        ...
        try {
            ...
            setShortFilms((prev) =>
                prev.map((f) =>
                    f.id === filmId ? { ...f, isPublished: !currentStatus } : f
                )
            );
            fetchStats();
            showToast(`Short film ${!currentStatus ? 'published' : 'unpublished'} successfully!`, 'success');
        } catch (error) {
    ...
    // ⭐ TOGGLE FEATURED
    // ═══════════════════════════════════════════════════════════════
    const handleToggleFeatured = async (filmId: string, currentStatus: boolean) => {
        if (!can(Permission.EDIT_ANY_CONTENT)) {
            showToast('You do not have permission to edit short films', 'error');
            return;
        }
        ...
        try {
            ...
            setShortFilms((prev) =>
                prev.map((f) =>
                    f.id === filmId ? { ...f, isFeatured: !currentStatus } : f
                )
            );
            fetchStats();
            showToast(`Short film ${!currentStatus ? 'featured' : 'unfeatured'} successfully!`, 'success');
        } catch (error) {
    ...
    // 🔍 FILTER SHORT FILMS
    // ═══════════════════════════════════════════════════════════════
    const filteredFilms = shortFilms.filter((film) =>
        film.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (film.titleHindi && film.titleHindi.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <AlertModal
                ...
                onClose={() => setAlertModal({ isOpen: false, filmId: '', filmTitle: '' })}
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-white/90 text-lg">Manage your short films collection</p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        <div className="bg-white/20 backdrop-blur-xl rounded-2xl p-5">
                            ...
                            <p className="text-4xl font-black text-amber-300">{stats.awarded}</p>
                        </div>
                        <div className="bg-white/20 backdrop-blur-xl rounded-2xl p-5">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4">
                    ...
                    />
                    <p className="text-slate-600 dark:text-slate-400 font-semibold">Loading short films...</p>
                </div>
                            ...
                            >
                                ...
                                <div className="relative aspect-[2/3] overflow-hidden bg-slate-200 dark:bg-slate-800">
                                    <img
                                        ...
                                        onError={(e) => {
                                            (e.target as HTMLImageElement).src =
                                                'https://via.placeholder.com/300x450/f97316/ffffff?text=' + encodeURIComponent(film.title);
                                        }}
                                    />
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <div className="absolute bottom-4 left-4 right-4">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-white font-bold">{formatRating(film.rating)}</span>
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        >
                                            <Play size={32} className="text-orange-600 ml-1" fill="currentColor" />
                                        </motion.button>
                                    </div>
                                </div>
                                        ...
                                        {can(Permission.EDIT_ANY_CONTENT) && (
                                            <>
                                                <motion.button
                                                    whileHover={{ scale: 1.05 }}
                                                    whileTap={{ scale: 0.95 }}
                                                    onClick={() => handleTogglePublish(film.id, film.isPublished || false)}
                                                    className={`flex-1 px-2 py-1 rounded-lg text-xs font-bold transition-all ${film.isPublished
                                                ...
                                                <motion.button
                                                    whileHover={{ scale: 1.05 }}
                                                    whileTap={{ scale: 0.95 }}
                                                    onClick={() => handleToggleFeatured(film.id, film.isFeatured || false)}
                                                    className={`flex-1 px-2 py-1 rounded-lg text-xs font-bold transition-all ${film.isFeatured
                ...
                >
                    <Video size={64} className="text-slate-300 dark:text-slate-700 mb-4" />
                    <h3 className="text-xl font-bold text-slate-600 dark:text-slate-400 mb-2">No Short Films Found</h3>
                    <p className="text-slate-500 dark:text-slate-500 mb-6">
                        {searchTerm ? 'Try different search terms' : 'Start by adding your first short film'}
                    </p>

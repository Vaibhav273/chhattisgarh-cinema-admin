...
import { logActivity, logError } from "../../utils/activityLogger";
              ...
              <div className="p-6 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 rounded-2xl border-2 border-orange-200 dark:border-orange-800">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-xl">
                    ...
                    <p className="font-bold text-slate-800 dark:text-white">
                      {new Date(payout.periodStartDate).toLocaleDateString(
                        "en-IN",
                        {
                          day: "2-digit",
                          month: "short",
                  ...
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-xl">
                    ...
                    <p className="font-bold text-slate-800 dark:text-white">
                      {new Date(payout.periodEndDate).toLocaleDateString(
                        "en-IN",
                        {
                          day: "2-digit",
                          month: "short",
              ...
              <div className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-2xl space-y-3 border-2 border-green-200 dark:border-green-800">
                ...
                <div className="flex justify-between items-center p-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
                  <span className="text-red-600 dark:text-red-400 font-semibold">
                    Platform Fee (
                    {(
                      (payout.earnings.platformFee /
                        payout.earnings.totalEarnings) *
                      100
                    ).toFixed(0)}
                    %)
                  </span>
        ...
        >
          ...
          <div className="p-6 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-700 rounded-b-3xl">
            ...
            <motion.button
              ...
              className={`px-6 py-3 text-white rounded-xl font-bold shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                action === "approve"
                  ? "bg-gradient-to-r from-green-500 to-emerald-600"
                  : "bg-gradient-to-r from-red-500 to-rose-600"
              }`}
  ...
  const handleApproveReject = async (payoutId: string, notes?: string) => {
    try {
      ...
      if (payoutToUpdate) {
        logActivity(
          "payout_action",
          `${actionModal.action === "approve" ? "Approved" : "Rejected"} payout for ${payoutToUpdate.creatorName}`,
          "Payouts",
          {
            action: actionModal.action,
            payoutId,
            creatorName: payoutToUpdate.creatorName,
            amount: payoutToUpdate.earnings.netPayout,
            period: payoutToUpdate.period,
            creatorId: payoutToUpdate.creatorId,
            creatorEmail: payoutToUpdate.creatorEmail,
            notes: notes,
            previousStatus: payoutToUpdate.status,
            newStatus:
      ...
      // ✅ ADD ERROR LOGGING
      logError("Payouts", `Failed to ${actionModal.action} payout`, {
        error,
        payoutId,
        action: actionModal.action,
      });

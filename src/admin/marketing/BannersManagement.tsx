...
const BannersManagement: React.FC = () => {
  ...
  const handleDelete = async (banner: Banner) => {
    ...
    try {
      ...
      // ✅ ADD LOGGING
      await logBannerAction("delete", banner.id, banner.title, {
      ...
      // ✅ ADD ERROR LOGGING
      await logError("Banners", "Failed to delete banner", {
        error,
        bannerId: banner.id,
    ...
    try {
      ...
      await logBannerAction(
        ...
        banner.title,
        {
          position: banner.position,
          priority: banner.priority,
          previousStatus: banner.isActive,
          newStatus: newStatus,
        },
      );
      ...
      // ✅ ADD ERROR LOGGING
      await logError("Banners", "Failed to toggle banner status", {
        error,
        bannerId: banner.id,
        bannerTitle: banner.title,
        attemptedStatus: !banner.isActive,
      });
    }
  };
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                      ...
                      <p className="text-2xl font-black text-green-600 dark:text-green-400">
                        {selectedBanner.views > 0
                          ? (
                              (selectedBanner.clicks / selectedBanner.views) *
                              100
                            ).toFixed(2)
                          : 0}
                        %

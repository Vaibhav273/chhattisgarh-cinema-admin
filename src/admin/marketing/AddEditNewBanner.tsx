...
import { logBannerAction, logError } from "../../utils/activityLogger";
...
const AddEditNewBanner: React.FC = () => {
  ...
  const handleImageUpload = (urls: { url: string; cdnUrl: string }[]) => {
  ...
  const handleMediaSelect = (url: string) => {
    setImagePreview(url);
    setImageMetadata({ url, path: "", size: 0, type: "image" });
    setErrors({ ...errors, image: undefined });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use imageMetadata URL directly
      let imageUrl = imageMetadata?.url || imagePreview;
      ...
      if (isEditMode) {
        ...
        // ✅ ADD LOGGING
        await logBannerAction("update", id!, formData.title.trim(), {
          position: formData.position,
          priority: formData.priority,
          isActive: formData.isActive,
          hasLink: !!formData.link,
          hasSchedule: !!(formData.startDate || formData.endDate),
        });
      } else {
        ...
        // ✅ ADD LOGGING
        await logBannerAction("create", docRef.id, formData.title.trim(), {
          position: formData.position,
          priority: formData.priority,
          isActive: formData.isActive,
          hasLink: !!formData.link,
      ...
      showToast(
        `Failed to ${isEditMode ? "update" : "create"} banner. Please try again.`,
        "error",
      );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <button
                    ...
                    className="w-full mt-4 px-6 py-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2"
                  >
                    <FolderOpen size={20} />
            ...
            <div className="space-y-6">
                ...
                <div className="p-6">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="w-5 h-5 text-pink-500 border-slate-300 dark:border-slate-700 rounded focus:ring-2 focus:ring-pink-500"
                    />
                    <div>
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-400">
                        Banner will be visible to users
                      </p>
                    </div>
                  </label>
                </div>
              ...
              <motion.button
                ...
                className="w-full px-8 py-4 bg-gradient-to-r from-pink-500 to-rose-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {uploading ? (
                  <>
                    <motion.div

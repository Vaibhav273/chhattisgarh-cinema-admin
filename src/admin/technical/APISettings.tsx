...
import { logAPIAction, logError } from "../../utils/activityLogger";
...
const APISettings: React.FC = () => {
  ...
  const handleGenerateKey = async (name: string, permissions: string[]) => {
    ...
    try {
      ...
      // ✅ ADD LOGGING
      await logAPIAction("create", "api_key", name, {
        keyId: docRef.id,
      ...
      // ✅ ADD ERROR LOGGING
      await logError("API Settings", "Failed to generate API key", {
        error,
        keyName: name,
      });
    }
  ...
  const handleRevokeKey = async (id: string) => {
    ...
    try {
      ...
      // ✅ ADD LOGGING
      await logAPIAction("delete", "api_key", keyToDelete?.name || "Unknown", {
        keyId: id,
      ...
      // ✅ ADD ERROR LOGGING
      await logError("API Settings", "Failed to revoke API key", {
        error,
        keyId: id,
      });
    }
  ...
  const handleSaveAPIConfig = async () => {
    ...
    try {
      ...
      // ✅ ADD LOGGING
      await logAPIAction("update", "api_config", "API Configuration", {
        baseUrl: apiConfig.baseUrl,
        version: apiConfig.version,
        rateLimit: apiConfig.rateLimit,
        corsEnabled: apiConfig.cors,
        docsEnabled: apiConfig.docs,
      });
    } catch (error) {
      ...
      // ✅ ADD ERROR LOGGING
      await logError("API Settings", "Failed to save API configuration", {
        error,
      });
    }
  };
  ...
  const handleSaveWebhook = async () => {
    ...
    try {
      ...
      // ✅ ADD LOGGING
      await logAPIAction("update", "webhook_config", "Webhook Configuration", {
        url: webhookConfig.url,
        enabledEvents: Object.entries(webhookConfig.events)
      ...
      // ✅ ADD ERROR LOGGING
      await logError("API Settings", "Failed to save webhook configuration", {
        error,
      });
    }
  };

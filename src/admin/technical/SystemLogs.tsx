...
import { logSystemAction } from "../../utils/activityLogger";
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div
            className={`bg-gradient-to-r ${getLevelColor(log.level)} p-6 text-white sticky top-0 z-10`}
          >
...
const SystemLogs: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const admins = [
      ...new Set(
        logs.map((log) => log.performedBy?.name || log.user).filter(Boolean),
      ),
    ].sort();
  ...
  useEffect(() => {
    ...
    if (filters.admin !== "all") {
      filtered = filtered.filter(
        (log) =>
          log.performedBy?.name === filters.admin || log.user === filters.admin,
      );
    }
  ...
  const handleExport = async () => {
    try {
      ...
      await logSystemAction(
        "system_logs_exported",
        "success",
        `Exported ${filteredLogs.length} system logs to CSV`,
        auth.currentUser,
        {
          totalLogs: filteredLogs.length,
          fileName: fileName,
          exportFormat: "CSV",
          activeFilters: hasActiveFilters() ? filters : "none",
          stats: {
            errors: stats.errors,
      ...
      // ✅ OPTIONAL: Log error
      await logSystemAction(
        "system_logs_export_failed",
        "failed",
        `Failed to export system logs: ${error instanceof Error ? error.message : "Unknown error"}`,
        auth.currentUser,
        {
          error: error instanceof Error ? error.message : "Unknown error",
          attemptedLogCount: filteredLogs.length,
        },
      );

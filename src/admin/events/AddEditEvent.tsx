...
const AddEditEvent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isEditMode && eventId) {
        ...
        try {
          ...
          if (eventDoc.exists()) {
            ...
            setFormData({
              ...
              descriptionHindi: eventData.descriptionHindi || "",
              image: eventData.image || "",
              imageCdnUrl: eventData.imageCdnUrl || "",
              bannerImage: eventData.bannerImage || "",
              bannerImageCdnUrl: eventData.bannerImageCdnUrl || "",
              galleryImages: eventData.galleryImages || [],
              galleryCdnUrls: eventData.galleryCdnUrls || [],
              date: eventData.date || "",
              ...
              venueHindi: eventData.venueHindi || "",
              venueAddress: eventData.venueAddress || "",
              venueAddressHindi: eventData.venueAddressHindi || "",
              city: eventData.city || "",
              cityHindi: eventData.cityHindi || "",
              state: eventData.state || "",
              stateHindi: eventData.stateHindi || "",
              pincode: eventData.pincode || "",
              venueCapacity: eventData.venueCapacity || 0,
              category: eventData.category || "film-festival",
              type: eventData.type || "in-person",
              organizer: eventData.organizer || "",
              organizerHindi: eventData.organizerHindi || "",
              ...
              isBookingOpen: eventData.isBookingOpen ?? true,
              minTicketsPerBooking: eventData.minTicketsPerBooking || 1,
              maxTicketsPerBooking: eventData.maxTicketsPerBooking || 10,
              language: eventData.language || "Hindi",
              languageHindi: eventData.languageHindi || "",
              ageRating: eventData.ageRating || "U",
              dresscode: eventData.dresscode || "",
              dresscodeHindi: eventData.dresscodeHindi || "",
              rules: eventData.rules || [],
              rulesHindi: eventData.rulesHindi || [],
              amenities: eventData.amenities || [],
              amenitiesHindi: eventData.amenitiesHindi || [],
              trailerUrl: eventData.trailerUrl || "",
              trailerCdnUrl: eventData.trailerCdnUrl || "",
              trailerMetadata: eventData.trailerMetadata,
              promoVideoUrl: eventData.promoVideoUrl || "",
              promoVideoCdnUrl: eventData.promoVideoCdnUrl || "",
              promoVideoMetadata: eventData.promoVideoMetadata,
              ...
              highlightsHindi: eventData.highlightsHindi || [],
              ...
              hasLiveStreaming: eventData.hasLiveStreaming || false,
              liveStreamUrl: eventData.liveStreamUrl || "",
              liveStreamCdnUrl: eventData.liveStreamCdnUrl || "",
              liveStreamMetadata: eventData.liveStreamMetadata,
              hasVIPMeetGreet: eventData.hasVIPMeetGreet || false,
              hasPhotoBooth: eventData.hasPhotoBooth || false,
              hasFoodStalls: eventData.hasFoodStalls || false,
              hasGiftBags: eventData.hasGiftBags || false,
            });
      ...
      console.log(
        "❌ Not calling fetchEvent - Edit mode:",
        isEditMode,
        "ID:",
        eventId,
      );
  ...
  const handleMainImageUpload = (urls: { url: string; cdnUrl: string }[]) => {
    if (urls.length === 0) return;

    const { url, cdnUrl } = urls[0]; // Take first image only

    console.log("📸 Main image uploaded:", { url, cdnUrl });
    ...
    logImageUpload(url, cdnUrl, "Events", "main", {
      eventTitle: formData.title || "Untitled Event",
      eventId: eventId || "new",
    });
  };

  const handleBannerImageUpload = (urls: { url: string; cdnUrl: string }[]) => {
    if (urls.length === 0) return;
    ...
    setFormData((prev) => ({
      ...prev,
      bannerImage: url,
      bannerImageCdnUrl: cdnUrl,
    }));

    showToast("Banner image uploaded successfully!", "success");
    logImageUpload(url, cdnUrl, "Events", "banner", {
      eventTitle: formData.title || "Untitled Event",
      eventId: eventId || "new",
    });
  };
  ...
  const handleGalleryImageUpload = (
    urls: { url: string; cdnUrl: string }[],
  ) => {
    console.log("📸 Gallery images received:", urls);

    setFormData((prev) => {
      // Check max limit
      if (prev.galleryImages && prev.galleryImages.length >= 10) {
      ...
      const newGalleryCdnUrls = [
        ...(prev.galleryCdnUrls || []),
        ...urls.map((u) => u.cdnUrl),
      ];

      console.log("✅ Gallery updated:", { total: newGalleryImages.length });
      logGalleryAdd(urls, "Events", newGalleryImages.length, {
        eventTitle: formData.title || "Untitled Event",
        eventId: eventId || "new",
      });
      ...
      return {
        ...
        galleryCdnUrls: newGalleryCdnUrls,
      };
    });

    showToast(`${urls.length} image(s) added!`, "success");
  };

  const removeGalleryImage = async (index: number) => {
    try {
      const imageUrl =
        formData.galleryCdnUrls?.[index] || formData.galleryImages?.[index];
      if (!imageUrl) return;
      ...
      const remainingImages = (formData.galleryImages?.length || 1) - 1;

      setFormData((prev) => ({
        ...prev,
        galleryImages: prev.galleryImages?.filter((_, i) => i !== index) || [],
        galleryCdnUrls:
          prev.galleryCdnUrls?.filter((_, i) => i !== index) || [],
      }));

      showToast("Image deleted successfully!", "success");

      // 🔥 LOG TO BOTH COLLECTIONS
      logGalleryRemove(imageUrl, index, "Events", remainingImages, {
        eventTitle: formData.title || "Untitled Event",
        eventId: eventId || "new",
      });
    } catch (error) {
      ...
      // 🔥 LOG ERROR TO BOTH COLLECTIONS
      logError("Events", "Failed to delete gallery image", { error, index });
    }
  };
  ...
  const handlePromoVideoUpload = (
    ...
    // ✅ ADD LOGGING
    logVideoUpload(url, encodedUrl || url, "Events", "promo", metadata || {}, {
      eventTitle: formData.title || "Untitled Event",
      eventId: eventId || "new",
      duration: metadata?.duration || "unknown",
      size: metadata?.size || "unknown",
  ...
  const handleLiveStreamUpload = (
    ...
    logVideoUpload(
      ...
      encodedUrl || url,
      "Events",
      "livestream", // ✅ CHANGED FROM "promo" TO "livestream"
      metadata || {},
      {
        eventTitle: formData.title || "Untitled Event",
        eventId: eventId || "new",
  ...
  const updateFAQ = (index: number, updates: Partial<FAQ>) => {
    setFormData((prev) => ({
      ...
      faq: prev.faq?.map((faqItem, i) =>
        i === index ? { ...faqItem, ...updates } : faqItem,
      ),
    }));
  };
  ...
  const updateMediaPartner = (
    index: number,
    updates: Partial<MediaPartner>,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && eventId) {
        ...
        // 🔥 LOG UPDATE TO BOTH COLLECTIONS
        logContentUpdate(
          "event",
          eventId,
        ...
        // 🔥 LOG CREATE TO BOTH COLLECTIONS
        logContentCreate(
          "event",
          docRef.id,
          formData.title || "Untitled Event",
      ...
      // 🔥 LOG ERROR TO BOTH COLLECTIONS
      logError("Events", "Failed to save event", { error });
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setFormData({ ...formData, descriptionHindi: e.target.value })
                }
                rows={4}
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
            ...
            <div className="md:col-span-2 flex flex-wrap gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      hasLiveStreaming: e.target.checked,
                    })
                  }
                  className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      hasVIPMeetGreet: e.target.checked,
                    })
                  }
                  className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      hasPhotoBooth: e.target.checked,
                    })
                  }
                  className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      hasFoodStalls: e.target.checked,
                    })
                  }
                  className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, hasGiftBags: e.target.checked })
                  }
                  className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                />
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <ImageUploader
                ...
                existingImageUrl={
                  formData.bannerImageCdnUrl || formData.bannerImage
                }
            ...
            <div>
              ...
              {formData.galleryCdnUrls &&
                formData.galleryCdnUrls.length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    {formData.galleryCdnUrls.map((url, index) => (
                      ...
                      >
                        {/* Image Preview */}
                        <div className="relative aspect-video rounded-xl overflow-hidden border-2 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                          <img
                          ...
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <span className="px-2 py-1 bg-white/20 backdrop-blur-sm rounded text-white text-xs font-bold">
                              #{index + 1}
                            </span>
                          </div>
                        </div>
                        ...
                        {/* Image Number Badge */}
                        <div className="absolute top-2 left-2 px-2 py-1 bg-purple-500 text-white text-xs font-bold rounded-lg">
                          {index + 1}
                        </div>
                      </motion.div>
                    ))}
              ...
              {/* Upload New Images */}
              {(!formData.galleryCdnUrls ||
                formData.galleryCdnUrls.length < 10) && (
                <ImageUploader
                ...
                formData.galleryCdnUrls.length >= 10 && (
                  <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl text-center">
                    <p className="text-sm font-semibold text-yellow-700 dark:text-yellow-400">
                      Maximum 10 images reached. Remove some to upload more.
                    </p>
                  </div>
                )}
            </div>
            ...
            <div>
              ...
              <VideoUploader
                ...
                existingVideoUrl={
                  formData.promoVideoCdnUrl || formData.promoVideoUrl
                }
            ...
            {formData.hasLiveStreaming && (
              <div>
                ...
                <VideoUploader
                  ...
                  existingVideoUrl={
                    formData.liveStreamCdnUrl || formData.liveStreamUrl
                  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, venueHindi: e.target.value })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    venueAddressHindi: e.target.value,
                  })
                }
                rows={2}
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, cityHindi: e.target.value })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, stateHindi: e.target.value })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    venueCapacity: parseInt(e.target.value) || 0,
                  })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, organizerHindi: e.target.value })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
          ...
          <div className="space-y-4">
            {formData.ticketTiers && formData.ticketTiers.length > 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateTicketTier(index, { nameHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateTicketTier(index, {
                            maxPerBooking: parseInt(e.target.value) || 10,
                          })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="10"
              ...
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                ...
                <p className="text-sm">
                  Click "Add Tier" to create ticket options
                </p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, bookingEndDate: e.target.value })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    minTicketsPerBooking: parseInt(e.target.value) || 1,
                  })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
                placeholder="1"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    maxTicketsPerBooking: parseInt(e.target.value) || 10,
                  })
                }
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500"
                placeholder="10"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateHost(index, { nameHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={host.role}
                        onChange={(e) =>
                          updateHost(index, { role: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateHost(index, { profileImage: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="https://..."
                      />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        value={host.bio}
                        onChange={(e) =>
                          updateHost(index, { bio: e.target.value })
                        }
                        rows={2}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        value={host.bioHindi}
                        onChange={(e) =>
                          updateHost(index, { bioHindi: e.target.value })
                        }
                        rows={2}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updatePerformer(index, { name: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updatePerformer(index, { nameHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        value={performer.bio}
                        onChange={(e) =>
                          updatePerformer(index, { bio: e.target.value })
                        }
                        rows={2}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="Brief bio"
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updatePerformer(index, { bioHindi: e.target.value })
                        }
                        rows={2}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="संक्षिप्त परिचय"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { nameHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { titleHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { organization: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, {
                            organizationHindi: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { topicHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { sessionTime: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { profileImage: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="https://..."
                      />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateSpeaker(index, { bioHindi: e.target.value })
                        }
                        rows={2}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="संक्षिप्त परिचय"
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateFAQ(index, { questionHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateFAQ(index, { answerHindi: e.target.value })
                        }
                        rows={3}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateSponsor(index, { nameHindi: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <select
                        value={sponsor.tier}
                        onChange={(e) =>
                          updateSponsor(index, { tier: e.target.value as any })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        value={partner.name}
                        onChange={(e) =>
                          updateMediaPartner(index, { name: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={partner.logo}
                        onChange={(e) =>
                          updateMediaPartner(index, { logo: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                        placeholder="https://..."
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateMediaPartner(index, { website: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
